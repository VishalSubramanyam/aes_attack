...
__global__ void
AES_ecb_encrypt_kernel (const u8 *in_d, u8 *out_d, unsigned long block_count)
{
 ...
 */
void
AES_ecb_128_encrypt_gpu (const u8          *in_h,
                         u8                *out_h,
                         ...
                         const unsigned int threads_per_blk)
{
    ...
    u8 *out_d;
    cudaMalloc (&in_d,
                aes_block_count * 16); // 1 block -> 128 bits -> 16 bytes
    cudaMalloc (&out_d,
                aes_block_count * 16); // 1 block -> 128 bits -> 16 bytes

    // copy the input data to the GPU
    cudaMemcpy (in_d, in_h, aes_block_count * 16, cudaMemcpyHostToDevice);

    // copy the expanded key to the GPU's constant memory
    cudaMemcpyToSymbol (globalKey, expanded_key_h, sizeof (AES_KEY));
    ...
    if (stream == 0)
        {
            AES_ecb_encrypt_kernel<<<num_cuda_blks, threads_per_blk>>> (
                in_d, out_d, aes_block_count);
        }
    else
        {
            AES_ecb_encrypt_kernel<<<num_cuda_blks,
                                     threads_per_blk,
                                     0,
                                     stream>>> (
                in_d, out_d, aes_block_count);
        }
    cudaMemcpy (out_h, out_d, aes_block_count * 16, cudaMemcpyDeviceToHost);
    cudaFree (in_d);
    cudaFree (out_d);
}
